...
const PrintPdf={
  request(sheet,title,text){
    const scoreSource=String(sheet&&sheet._text||text||''),scoreIssues=[...(typeof scoringIntegrityIssues==='function'?scoringIntegrityIssues(scoreSource):[]),...scoringCompletenessIssues(scoreSource,sheetScoringMode(sheet))];
...
function makeSheet(key,loading){
  ...
  sheet.className='sheet';sheet.dataset.t=t.color;sheet._tierKey=key;sheet._text='';sheet._key='';sheet._quality='';sheet._qualityStage='none';sheet._qualityApplied=[];sheet._finalAuditUsed=false;sheet._scoringMode='none';sheet._manualScores={};sheet._structureContract=null;sheet._visualAssets=[];sheet._mediaSource=null;sheet._parts={title:'',instructions:'',tasks:'',answerKey:'',teacherNote:''};sheet._structured=false;sheet._validation={ok:true,issues:[]};sheet._pdfWarningSkipped=false;

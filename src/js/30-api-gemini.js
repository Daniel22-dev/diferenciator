...
function applyVisualManifest(entries,documentEntries,scanReports=[]){
  ...
  sourceVisualAssets=sourceVisualAssets.map(asset=>{
    ...
    return {...asset,role,type,intent,confidence,description:String(meta.description||meta.desc||'').slice(0,500),mode:asset.modeTouched?normalizeVisualMode(asset.mode):defaultVisualMode(role,intent)};
  });
...
function renderSourceVisualPanel(){
  const panel=$('#visualSourcePanel'),list=$('#visualSourceList'),summary=$('#visualSourceSummary'),badge=$('#visualSourceBadge'),warning=$('#visualSourceWarning'),supp=$('#visualSupplementBtn'),applyAll=$('#visualApplyRecommendedBtn');if(!panel||!list)return;
  ...
  if(sourceVisualAssets.some(a=>a.quality&&a.quality.warnings&&a.quality.warnings.length))notes.push('Lokální kontrola obrazu našla potenciální problém s kvalitou alespoň u jednoho snímku. „Vylepšit čitelnost pro AI“ mění jen čtecí kopii.');
...
function declaredScoreTotals(text){const rx=/^(?:\s*\*{0,2})?(?:celkem|total|součet|soucet|maximum|max\.?)\s*[:=]?\s*(\d+(?:[.,]\d+)?)\s*(?:bod(?:ů|u|y)?|b\.?|points?|pts?)(?:\*{0,2})?\s*$/gim;return [...String(text||'').matchAll(rx)].map(m=>Number(String(m[1]).replace(',','.'))).filter(Number.isFinite)}

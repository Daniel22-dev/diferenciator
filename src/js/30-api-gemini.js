...
function normalizeVisualItemCounts(value){const src=Array.isArray(value)?value:String(value??'').split(',');return src.map(x=>Number.parseInt(String(x).trim(),10)).filter(n=>Number.isInteger(n)&&n>0&&n<=200).slice(0,20)}
...
function sourceStructureContract(key='core'){
  ...
  return {enforce:!!strict,itemCounts,expectedMainTasks:itemCounts.length,explicitExamples:examplesKnown?exampleValues.reduce((a,b)=>a+b,0):null,allowExtensions:!!a.allowExtensions,variantMode:String(a.variantMode||'auto')};
}
...
function generatedMainTaskItemCounts(text){return generatedMainTaskBlocks(text).map(block=>{const lines=String(block.text||'').split(/\r?\n/);lines.shift();const nums=[];for(const line of lines){const m=String(line||'').trim().replace(/^\*{1,2}/,'').match(/^(\d{1,3})\s*(?:[.)-]|:)\s+\S/);if(m)nums.push(Number(m[1]))}return new Set(nums).size})}
...
function renderSourceVisualPanel(){
  ...
  if(applyAll)applyAll.onclick=()=>{for(const asset of sourceVisualAssets){asset.mode=visualRecommendedMode(asset);asset.modeTouched=true}renderSourceVisualPanel();showMessage('Doporučené režimy nastaveny','Úlohy zachycené obrazem se převedou do nové editovatelné podoby; skutečné mapy, grafy, fotografie a schémata se zachovají jako podklady.');};
  ...
  const taskCount=sourceVisualAssets.filter(a=>a.intent==='task_image').length,contentCount=sourceVisualAssets.filter(a=>a.intent==='content_visual').length,hybridCount=sourceVisualAssets.filter(a=>a.intent==='hybrid').length;
  if(summary){
    const intents=[];if(taskCount)intents.push(taskCount+'× úloha v obrázku');if(contentCount)intents.push(contentCount+'× skutečný obrazový podklad');if(hybridCount)intents.push(hybridCount+'× kombinovaný podklad');
    if(intents.length)summary.textContent='Rozpoznáno: '+intents.join(' · ')+'. Doporučený režim je už předvolený.';
  ...
  for(let index=0;index<sourceVisualAssets.length;index++){
    ...
    const recommended=visualRecommendedMode(asset),rec=document.createElement('div');rec.className='visual-source-recommendation';rec.textContent='Doporučeno: '+visualModeLabel(recommended)+(asset.intent==='hybrid'?' — pokud žák potřebuje skutečnou obrazovou část, vyřízni a zachovej jen ji.':'');
    const controls=document.createElement('div');controls.className='visual-source-controls';const lab=document.createElement('label');lab.textContent='Použití';const sel=document.createElement('select');sel.id='visualMode_'+asset.id;lab.htmlFor=sel.id;sel.dataset.visualMode=asset.id;[['reconstruct','Převést na novou diferencovanou úlohu'],['preserve','Použít jako původní obrazový podklad'],['reference','Použít jen jako referenci'],['ignore','Nepoužívat']].forEach(([v,t])=>{const o=document.createElement('option');o.value=v;o.textContent=t+(v===recommended?' · doporučeno':'');if(asset.mode===v)o.selected=true;sel.appendChild(o)});sel.addEventListener('change',()=>{asset.mode=normalizeVisualMode(sel.value);asset.modeTouched=true;renderSourceVisualPanel()});controls.append(lab,sel);
    ...
    if(sourceVisualAssets.length>1){const earlier=document.createElement('button');earlier.type='button';earlier.className='btn ghost small';earlier.textContent='↑ Dříve';earlier.disabled=index===0;earlier.title='Posune obraz dříve v pořadí.';earlier.addEventListener('click',()=>moveVisualAsset(asset.id,-1));const later=document.createElement('button');later.type='button';later.className='btn ghost small';later.textContent='↓ Později';later.disabled=index===sourceVisualAssets.length-1;later.title='Posune obraz později v pořadí.';later.addEventListener('click',()=>moveVisualAsset(asset.id,1));controls.append(earlier,later)}
    copy.append(title,meta,rec,controls);card.append(prev,copy);list.appendChild(card);
  }
  const notes=[];
...
function declaredScoreTotals(text){const rx=/^(?:\s*\*{0,2})?(?:celkem(?:\s+bod(?:ů|u|y)?)?|total(?:\s+(?:points?|pts?))?|součet|soucet|maximum|max\.?)\s*[:=]?\s*(\d+(?:[.,]\d+)?)(?:\s*(?:bod(?:ů|u|y)?|b\.?|points?|pts?))?\.?(?:\*{0,2})?\s*$/gim;return [...String(text||'').matchAll(rx)].map(m=>Number(String(m[1]).replace(',','.'))).filter(Number.isFinite)}

...
function educationalVisualPromptLines(subject){
  const kind=subjectDomainKind(subject),stem=typeof stemSubjectKind==='function'?stemSubjectKind(subject):'',base='ODBORNÉ VIZUÁLY 1.3.47: nový přesný vizuál zapisuj jako samostatný [[EDU_TYP|{JSON}]]. Data markeru musí souhlasit se zadáním i klíčem. Zachovaný [[VISUAL_n]] nenahrazuj aproximací.';
  ...
  const arith='EDU_ARITH: {\"operation\":\"subtract|add|multiply|divide\",\"columns\":4,\"items\":[[38,19],[61,25],[145,77]]}. Each pair is one vertical item; never repeat horizontally; preserve item/column counts.';
  ...
  if(kind==='stem'){
    if(stem==='math')return [base,chart,math,arith,coord,flow];
...
function educationalVisualIssues(text,assetIds=null){
  const issues=[],allowedAssets=Array.isArray(assetIds)?new Set(assetIds.map(x=>String(x).toUpperCase())):null;for(const line of String(text||'').split(/\r?\n/)){if(!/\[\[EDU_/.test(line))continue;const info=educationalMarkerInfo(line);if(!info){issues.push('Odborný vizuální marker má neplatný formát, typ nebo není na samostatném řádku.');continue}if(info.error){issues.push('Odborný vizuální marker '+info.type+' neobsahuje platný JSON.');continue}const spec=info.spec||{};
    ...
    if(info.kind==='arith'){const op=String(spec.operation||'').toLowerCase(),items=Array.isArray(spec.items)?spec.items:[],cols=Number(spec.columns);if(!['subtract','add','multiply','divide'].includes(op)||!items.length||items.length>200||!items.every(x=>Array.isArray(x)&&x.length===2&&x.every(n=>Number.isFinite(Number(n))))||!Number.isInteger(cols)||cols<1||cols>8)issues.push('EDU_ARITH potřebuje platnou operaci, 1–200 číselných dvojic a 1–8 sloupců.');}
...
function appendEducationalRichText(parent,text,kind=typeof stemSubjectKind==='function'?stemSubjectKind():null,assets=[]){
  ...
  while(i<lines.length){
    if(/^\s*-{3,}\s*$/.test(lines[i])){flush();const hr=document.createElement('hr');hr.className='worksheet-separator';parent.appendChild(hr);i++;continue}

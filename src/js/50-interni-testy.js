globalThis.TestSystem={
  ...
  runParserTests(){
    ...
    const floatingOrder=docxReferencedMediaPaths('<w:p><wp:anchor><wp:positionV relativeFrom="paragraph"><wp:posOffset>248285</wp:posOffset></wp:positionV><a:blip r:embed="rId4"/></wp:anchor><wp:anchor><wp:positionV relativeFrom="paragraph"><wp:posOffset>7952105</wp:posOffset></wp:positionV><a:blip r:embed="rId5"/></wp:anchor><wp:anchor><wp:positionV relativeFrom="paragraph"><wp:posOffset>7275830</wp:posOffset></wp:positionV><a:blip r:embed="rId5"/></wp:anchor><wp:anchor><wp:positionV relativeFrom="paragraph"><wp:posOffset>4104005</wp:posOffset></wp:positionV><a:blip r:embed="rId6"/></wp:anchor></w:p>','<Relationships><Relationship Id="rId4" Type="x/image" Target="media/image1.png"/><Relationship Id="rId5" Type="x/image" Target="media/image2.png"/><Relationship Id="rId6" Type="x/image" Target="media/image3.jpeg"/></Relationships>');
    this.assert(floatingOrder.join(',')==='word/media/image1.png,word/media/image3.jpeg,word/media/image2.png','DOCX plovoucí obrázky podle pozice','Více plovoucích obrázků v jednom Word odstavci se řadí podle svislé pozice a duplicity se odstraní.','DOCX parser stále používá pořadí vložení místo vizuálního pořadí plovoucích obrázků.');
    ...
    this.assert(!/změň konkrétní obsah/i.test(batchPrompt)&&/referenční standard/i.test(batchPrompt),'Normální verze v celé sadě','Automatická Normální verze zachovává obsah, strukturu a obtížnost jako referenční standard.','Normální verze sady stále žádá jiný obsah nebo nemá jasný referenční význam.');
    this.assert(!/změň konkrétní obsah/i.test(singlePrompt)&&/referenční standard/i.test(singlePrompt),'Samostatná Normální verze','Samostatná i dávková Normální verze mají stejný referenční význam; nový paralelní obsah je explicitní režim.','Samostatná Normální verze se stále významově liší od celé sady.');
    const oldVariant=$('#advVariantMode')?$('#advVariantMode').value:'auto',oldStructure=$('#advStructureMode')?$('#advStructureMode').value:'auto';
    if($('#advVariantMode'))$('#advVariantMode').value='same_format_new_content';if($('#advStructureMode'))$('#advStructureMode').value='flexible';
    this.assert(!sourceStructureContract('core').enforce,'Explicitní Flexible má přednost','Volba Flexible skutečně vypne deterministické vynucení struktury i u režimu „Stejný formát, jiný obsah“.','Flexible je v UI zvoleno, ale strukturální kontrakt se stále vynucuje.');
    if($('#advVariantMode'))$('#advVariantMode').value=oldVariant;if($('#advStructureMode'))$('#advStructureMode').value=oldStructure;syncVariantTierRules();
  },
  ...
  runStemSafetyTests(){
    ...
    try{
      ...
      const bracketEqCorrect=['1. (x + 1) : 5 = 2 → x = 9','2. (x + 3) : 6 = 3 → x = 15','3. 9 - (x - 2) = 3 → x = 8','4. 9x = 3(-4 + 5x) → x = 2','5. 3(6 - x) = 33 → x = -5','6. 75 = 3(5 - 5x) → x = -4','7. 5 · (x - 1) = 15 → x = 4','8. -34 = -(3x - 8) → x = 14','9. 10 - (x + 1) = 6 → x = 3','10. 8(x + 1) = 2(x + 16) → x = 4','11. -(4 + x) = 3x → x = -1','12. 5(x - 2) = 3(x + 4) → x = 11'];
      const bracketEqWrong=['1. (x + 1) : 5 = 2 → x = 8','2. (x + 3) : 6 = 3 → x = 14','3. 9 - (x - 2) = 3 → x = 7','4. 9x = 3(-4 + 5x) → x = 3','5. 3(6 - x) = 33 → x = -4','6. 75 = 3(5 - 5x) → x = -3','7. 5 · (x - 1) = 15 → x = 5','8. -34 = -(3x - 8) → x = 13','9. 10 - (x + 1) = 6 → x = 4','10. 8(x + 1) = 2(x + 16) → x = 5','11. -(4 + x) = 3x → x = 0','12. 5(x - 2) = 3(x + 4) → x = 10'];
      this.assert(stemEvalArithmetic('10:5')===2&&bracketEqCorrect.every(x=>stemLinearEquationIssues(x).length===0)&&bracketEqWrong.every(x=>stemLinearEquationIssues(x).length===1),'Rovnice se závorkami — STEM','Česká dvojtečka, závorky, implicitní násobení, záporné členy a očíslovaný klíč se ověřují bez false-positive; všech 12 chybných řešení je zachyceno.','STEM kontrola selhala na praktickém PDF Rovnice se závorkami.');
  ...
  async runPdfFlowTest(){
    ...
    const previousStructureReport=sourceStructureReport?{...sourceStructureReport,itemCounts:[...(sourceStructureReport.itemCounts||[])]}:null;
    const structureManifest=splitVisualManifest(['Rovnice se závorkami','<<<VISUAL_MANIFEST>>>','SOURCE_STRUCTURE|task_item_counts=12|explicit_examples=0|description=jedna hlavní úloha se 12 rovnicemi','SCAN_REPORT|status=good|pages=1|issues=none','<<<END_VISUAL_MANIFEST>>>'].join('\n'));
    applyVisualManifest(structureManifest.entries,structureManifest.documentEntries,structureManifest.scanReports,structureManifest.sourceStructures);
    const mathTable='**1. Vyřeš rovnice.**\n| Levý sloupec | Pravý sloupec |\n|---|---|\n| (x+1):5=2 | (x+3):6=3 |\n| 9-(x-2)=3 | 9x=3(-4+5x) |\n| 3(6-x)=33 | 75=3(5-5x) |\n| 5(x-1)=15 | -34=-(3x-8) |\n| 10-(x+1)=6 | 8(x+1)=2(x+16) |\n| -(4+x)=3x | 5(x-2)=3(x+4) |';
    const mathSubjectBefore=subject?subject.value:'';if(subject)subject.value='Matematika';
    const pdfContract=sourceStructureContract('core'),mathTableCounts=generatedMainTaskItemCounts(mathTable);
    this.assert(pdfContract.itemCounts.length===1&&pdfContract.itemCounts[0]===12&&pdfContract.explicitExamples===0,'PDF SOURCE_STRUCTURE kontrakt','Struktura celé PDF stránky se 12 rovnicemi se zachová i bez VISUAL_n assetu.','Pure-PDF struktura se z manifestu nepřenesla do kontraktu.');
    this.assert(mathTableCounts.length===1&&mathTableCounts[0]===12,'Dvousloupcová matematická tabulka','Strukturální validátor spočítá 12 rovnic i v přirozeném dvousloupcovém Markdown layoutu.','Strukturální validátor neumí spočítat matematické položky v tabulce.');
    const singleton24=Array.from({length:24},()=>1).join(','),arithManifest=splitVisualManifest(['Písemné odčítání','<<<VISUAL_MANIFEST>>>','SOURCE_STRUCTURE|task_item_counts='+singleton24+'|layout=vertical_arithmetic_grid|columns=4|rows=6|explicit_examples=0|description=24 příkladů v mřížce 4 × 6','SCAN_REPORT|status=good|pages=1|issues=none','<<<END_VISUAL_MANIFEST>>>'].join('\n'));
    applyVisualManifest(arithManifest.entries,arithManifest.documentEntries,arithManifest.scanReports,arithManifest.sourceStructures);
    const arithItems=Array.from({length:24},(_,i)=>[100+i,20+i]),arithText='**1. Vypočítejte písemně.**\n[[EDU_ARITH|'+JSON.stringify({operation:'subtract',columns:4,items:arithItems})+']]',arithContract=sourceStructureContract('core'),arithCounts=generatedMainTaskItemCounts(arithText),arithIssues=structurePreservationIssues(arithText,arithContract),arithPrompt=sourceStructurePromptLines().join(' ');
    this.assert(arithContract.itemCounts.length===1&&arithContract.itemCounts[0]===24&&arithContract.layout==='vertical_arithmetic_grid'&&arithContract.columns===4,'Písemné odčítání — normalizace struktury','Mřížka 4 × 6 se chápe jako jedna hlavní úloha s 24 položkami, i když extraktor vrátí jednotlivé jedničky.','Mřížka písemného odčítání se stále rozpadá na falešné hlavní úlohy.');
    this.assert(arithCounts.length===1&&arithCounts[0]===24&&arithIssues.length===0&&/NEPIŠ současně/.test(arithPrompt),'Písemné odčítání — EDU_ARITH','24 svislých příkladů v EDU_ARITH projde Strict kontraktem a prompt zakazuje horizontální duplikaci.','EDU_ARITH není započítán do struktury nebo prompt stále dovoluje duplicitní zápis.');
    const duplicatedArithmetic='**1. Vypočítejte písemně.**\n'+arithItems.map((x,i)=>(i+1)+') '+x[0]+' - '+x[1]+' =\n'+x[0]+'\n- '+x[1]).join('\n');
    this.assert(structurePreservationIssues(duplicatedArithmetic,arithContract).some(x=>/EDU_ARITH/.test(x)),'Písemné odčítání — blokace rozbitého layoutu','Strict režim odmítne dlouhý duplicitní horizontální+svislý zápis místo původní mřížky.','Rozbitý jedno-sloupcový duplikovaný layout může projít jako zachovaná struktura.');
    if(subject)subject.value=mathSubjectBefore;sourceStructureReport=previousStructureReport;
    ...
    const strippedEnglishTotal=stripGeneratedScoring('**Exercise 1 (5 points)**\n1. A\nTotal points: 5');
    this.assert(!/points?/i.test(strippedEnglishTotal)&&/Exercise 1/.test(strippedEnglishTotal),'Odstranění anglického celkového součtu','Režim Ruční/Bez bodování odstraní i samostatný řádek „Total points: N“, ne pouze bodový suffix.','Po deterministickém odstranění bodování zůstává anglický řádek Total points.');
    const noScoreScreenshot=normalizeParsedScoring({worksheet:'Písemné odčítání',parts:{title:'Písemné odčítání',instructions:'Vypočítejte následující příklady na odčítání. Příklady zapište písemně pod sebe do připraveného prostoru a vypočítejte. Na vypracování máte 25 minut. Hodnocení: 1 bod za každý správný výsledek (celkem 24 bodů).',tasks:'**1. Vypočítejte následující rozdíly písemně pod sebou (každý příklad = 1 bod, celkem 24 bodů):**\n[[EDU_ARITH|{"operation":"subtract","columns":4,"items":[[38,19],[61,25]]}]]'}},'none');
    const noScoreScreenshotText=[noScoreScreenshot.parts.instructions,noScoreScreenshot.parts.tasks].join(' ');
    this.assert(/25 minut/.test(noScoreScreenshot.parts.instructions)&&!/Hodnocení|\d+\s*bod|celkem\s+24\s+bod/i.test(noScoreScreenshotText),'Bez bodování — prose scoring leak','Výchozí „Bez bodování“ odstraní i AI větu „Hodnocení: 1 bod…“ a složenou bodovací závorku, ale zachová časový limit.','Do žákovské verze bez bodování stále prosakuje slovní bodování z AI výstupu.');
    const inlineTotal='Instructions. Time limit: 30 minutes. Total points: 34.\n\n**1. A (10 points)**\nText\n\n**2. B (24 points)**\nText';
    this.assert(declaredScoreTotals(inlineTotal).includes(34)&&scoringCompletenessIssues(inlineTotal,'ai').length===0&&scoringIntegrityIssues(inlineTotal).length===0,'Celkový součet uvnitř instrukce','„Total points: 34“ se rozpozná i uprostřed instrukční věty a PDF se falešně nezablokuje.','Inline celkový součet stále vyvolává false-positive scoring gate.');
    const scoringEdit=makeSheet('core',false);scoringEdit._structured=true;scoringEdit._scoringMode='ai';scoringEdit._parts={title:'Body',instructions:'Work carefully. Total points: 9.',tasks:'**1. First (4 points)**\nText\n\n**2. Second (5 points)**\nText',answerKey:'1. A\n2. B',teacherNote:''};scoringEdit._text=[scoringEdit._parts.title,scoringEdit._parts.instructions,scoringEdit._parts.tasks].join('\n\n');scoringEdit._validation={ok:true,issues:[]};const editTotal=applyEmbeddedScoring(scoringEdit,{0:6,1:5});this.assert(editTotal===11&&/Total points: 11/.test(scoringEdit._parts.instructions)&&/First \(6 points\)/.test(scoringEdit._parts.tasks)&&scoringIntegrityIssues(scoringEdit._text).length===0,'Lokální editor bodování','Změní body hlavních úloh, automaticky přepočítá součet a nepotřebuje AI.','Editor bodování nepřepsal konzistentně úlohy a celkový součet.');
    const noteSheet=makeSheet('core',false);noteSheet._targetGroup='8g-sexta';noteSheet._cefrLabel='B2';noteSheet._parts.teacherNote='Target audience: Sexta (gymnasion, 16-17 years old), B2 CEFR level. Keep the worksheet challenging.';renderTeacherNote(noteSheet);this.assert(!/gymnasion/i.test(noteSheet.querySelector('.teacherbox').textContent)&&/Sexta/.test(noteSheet.querySelector('.teacherbox').textContent)&&/16–17 let/.test(noteSheet.querySelector('.teacherbox').textContent),'Deterministická cílová skupina učitele','Volná AI formulace target audience je nahrazena školními daty a překlep gymnasion se nezobrazí.','Poznámka pro učitele stále zobrazuje chybnou volnou formulaci cílové skupiny.');
    const mdProbe=document.createElement('div');appendEducationalRichText(mdProbe,'*Scenario:* test\n---\nCompare *blushing* with *fidgeting*.','language',[]);this.assert(mdProbe.querySelectorAll('em').length===3&&mdProbe.querySelector('hr.worksheet-separator'),'Markdown cleanup','Jednoduchá kurzíva se vykreslí a --- se změní na oddělovač.','Ve výstupu zůstávají literální markdownové hvězdičky nebo --- separator.');
  },
  runLayoutTest(){
